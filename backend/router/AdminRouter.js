...
router.post("/verify_account", async (req, res, next) => {
  ...
  jwt.verify(token, process.env.jwt_key, async (err, valid_token) => {
    ...
    const findAccount = await AdminModel.findById(id);
...
router.post(
  ...
  async (req, res, next) => {
    ...
    const findone = await AdminModel.findOne({ username: username });
    ...
    await bcrypt.compare(password, findone.password, (err, isValid) => {
      if (isValid) {
        ...
        res
          ...
          .status(200)
          .send({ message: "Loggin ", token, created: true, id, username });
      } else {
...
router.post(
  ...
  async (req, res, next) => {
    ...
    const findOne_username = await AdminModel.findOne({ username: username });
    const findOne_email = await AdminModel.findOne({ email: email });
