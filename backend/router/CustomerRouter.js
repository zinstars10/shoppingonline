...
router.post("/verify_account", async (req, res, next) => {
  ...
  jwt.verify(token, process.env.jwt_key, async (err, valid_token) => {
    ...
    const findAccount = await customersModel.findById(id);
...
router.post(
  ...
  async (req, res, next) => {
    ...
    const findone = await customersModel.findOne({ username: username });
    ...
    await bcrypt.compare(password, findone.password, (err, isValid) => {
      if (isValid) {
        ...
        res
          ...
          .status(200)
          .send({ message: "Loggin ", token, created: true, id });
      } else {
...
router.post(
  ...
  async (req, res, next) => {
    const { name, email, username, password, confirm_password } =
      req.body;
    ...
    const findOne_username = await customersModel.findOne({ username: username });
    const findOne_email = await customersModel.findOne({ email: email });
